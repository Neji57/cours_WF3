...
function getLastMessages($pdo, $lastId)
{
    ...
    {
        $count = 5;
        $query = $pdo->prepare("SELECT message.*, user.username, user.avatar FROM message JOIN user On user.id = message.user WHERE message.id > :lastId ORDER BY id DESC LIMIT :limit");
